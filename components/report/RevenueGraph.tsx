...
{(() => {
  ...
  return (
    <p className={`text-xs px-2 py-1 rounded inline-block mt-2.5 lg:mt-0 ${isPositive ? 'bg-green-100' : 'bg-red-100'} ${colorClass}`}>
      {arrow} {Math.abs(percent)}% vs. {previous} last week
    </p>
  );
})()}
      {isRevenueLoading && <p className="text-sm text-gray-500 mb-2">Loading data...</p>}
      {isRevenueError && <p className="text-sm text-red-500 mb-2">Error loading data</p>}
